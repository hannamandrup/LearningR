...
tidier_post_meal_data %>%
  mutate(insulin_glucose_ratio = auc_insulin / auc_glucose) # Makes a new column called 'insulin_glucose_ratio' in which the ratio of auc_insulin and auc_glucose is found

tidier_post_meal_data %>%
  mutate(
    insulin_glucose_ratio = auc_insulin / auc_glucose,
    Group = if_else(Group == "CTR", "Control", "First-degree relative")
  ) # If rows within the "Group"-column has "CTR" (aka TRUE), this is exchanged with "Control", and if this is FALSE (?), the row will be called "First-degree relative"
```
...
tidied_post_meal_data %>%
  ...
  geom_jitter() # Also possible to add multiple layers on top of each other, simply add a '+' in the end, go to new line and add, i.e., a violin plot on top
```
...
tidied_post_meal_data %>%
  ...
  geom_point() +
  facet_grid(rows = vars(young)) + # stratifies the groups (cols/rows= columns/rows and vars= variable)
...
post_meal_data |>
  ...
  scale_fill_viridis_d(option = "cividis") # Viridis is colour-blindness friendly, '_d' indicates that data is discrete. Nothing is neded within the parentheses

# Tasks 3. and 4.
post_meal_data |>
  ggplot(aes(x = auc_pg, y = auc_ins, colour = BMI)) +
  geom_point() +
  scale_colour_viridis_c(option = "inferno") # '_c' indicates that data is continuous. Nothing is neded within the parentheses
```
